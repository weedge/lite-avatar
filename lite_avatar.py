...
class liteAvatar(object):
    def __init__(
        self,
        data_dir=None,  # avatar data dir (mouth pos)
        num_threads=1,
        ...
        logging.info("liteAvatar init start...")
        ...
        self.audio2mouth = Audio2Mouth(use_gpu=use_gpu, weight_dir=weight_dir)
        logging.info(f"audio2mouth init over in {time.time() - s}s")

        self.input_queue = queue.Queue()
        self.output_queue = queue.Queue()
        self.load_data_thread: threading.Thread = None

        logging.info("liteAvatar init over")
        ...
        if generate_offline:
            ...
            for i in range(num_threads):
                t = threading.Thread(
                    target=self.face_gen_loop,
                    args=(i, barrier_prep, self.input_queue, self.output_queue),
                )
    ...
    def load_dynamic_model(self, data_dir):
        logging.info("start to load dynamic data")
    ...
    def load_data_sync(self, data_dir, bg_frame_cnt=None):
        ...
        while True:
            ...
            if ret is False:
                break
        self.bg_video_frame_count = (
            len(self.bg_data_list)
            if bg_frame_cnt is None
            else min(bg_frame_cnt, len(self.bg_data_list))
        )

        y1, y2, x1, x2 = open(f"{data_dir}/face_box.txt", "r").readlines()[0].split()
        self.y1, self.y2, self.x1, self.x2 = int(y1), int(y2), int(x1), int(x2)

        self.merge_mask = (np.ones((self.y2 - self.y1, self.x2 - self.x1, 3)) * 255).astype(
            np.uint8
        )
    ...
    def load_data(self, data_dir, bg_frame_cnt=None):
        ...
        logging.info(f"load data over in {time.time() - s}s")
    ...
    def face_gen_loop(self, thread_id, barrier, in_queue, out_queue):
        while True:
            ...
            logging.info(
                "global_frame_id: {} in {}s".format(global_frame_id, round(time.time() - s, 3))
            )
    ...
    def get_idle_param(self):
        ...
        tmp_json = {}
        for ii in range(len(self.audio2mouth.p_list)):
    ...
    def audio2param(
        self, input_audio_byte, prefix_padding_size=0, is_complete=False, audio_status=-1
    ):
        ...
        sound = AudioSegment.from_raw(
            BytesIO(input_audio_byte), sample_width=2, frame_rate=16000, channels=1
        )
        ...
        if len(start_end_list) > 0:
            for start, end in start_end_list:
                ...
                logging.info(f"silence part: {start_frame}-{end_frame} frames")
    ...
    def make_silence(self, param_res, sil_scale):
        ...
        for ii in range(len(param_res)):
            for key in param_res[ii]:
                ...
                param_res[ii][key] = (
                    param_res[ii][key] * (1 - sil_scale[ii]) + neu_value * sil_scale[ii]
                )
        return param_res
    ...
    def handle(self, audio_file_path, result_dir, param_res=None):
        ...
        cmd = "`which ffmpeg` -r 30 -i {}/%05d.jpg -i {} -framerate 30 -c:v libx264 -pix_fmt yuv420p -b:v 5000k -strict experimental -loglevel error {}/test_demo.mp4 -y".format(
            tmp_frame_dir, audio_file_path, result_dir
        )
        os.system(cmd)
